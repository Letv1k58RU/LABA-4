...
{
    TEST(ValidKey) { 
    wstring z1 = L"АМЖНСЁЛДЕМРЕ";
    wstring z2 = modAlphaCipher(L"АБВГ").encrypt(L"АЛЕКСЕЙБЕЛОВ");
    if (z1==z2) {
    CHECK(true);
    }
    else {
    ...
    TEST(LongKey) { 
    wstring z3 = L"РХФРЖ";
    wstring z4 = modAlphaCipher(L"ПРИВЕТКАКДЕЛАААА").encrypt(L"БЕЛОВ");
    if (z3==z4) {
    CHECK(true);
    }
    else {
    ...
    TEST(LowCaseKey) { 
    wstring z5 = L"АМЖНСЁЛДЕМРЕ";
    wstring z6 = modAlphaCipher(L"абвг").encrypt(L"АЛЕКСЕЙБЕЛОВ");
    if (z5==z6) {
    CHECK(true);
    }
    else {
